...
export function SpreadGallery({ selectedId, onSelect }: Props) {
  return (
      ...
      {tarotSpreads.map((spread, index) => (
        ...
        >
          <div className="flex items-center justify-between text-[10px] uppercase tracking-[.28em] text-antiqueGold/65">
            <span>0{index + 1} · {spread.category}</span>
          ...
          <h3 className="mt-2 font-zhSerif text-3xl tracking-[.08em] text-moon">{spread.name}</h3>
          <p className="mt-4 font-zhSerif text-sm leading-7 text-moon/58">{spread.description}</p>
          <div className="mt-6 flex items-center justify-between border-t border-white/[.06] pt-4 text-xs text-moon/45">
            ...
            <span className="text-antiqueGold transition group-hover:translate-x-1">选择牌阵 →</span>
          </div>
        </button>
      ))}
    </div>
