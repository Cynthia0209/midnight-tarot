    ...
    <>
      ...
      {savedReading && (
        <div className="mt-12 flex flex-wrap items-center gap-4 border-t border-antiqueGold/10 pt-7">
          ...
          <span className="text-[11px] text-moon/35">{locale === "en" ? "Saved in this browser · Up to 20 readings" : "已保存在此浏览器 · 最多保留 20 次"}</span>
    ...
    <div className="mx-auto max-w-6xl">
      <div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr]">
        ...
        <article className="pb-20">
          ...
          {context && <p className="mt-4 border-l border-antiqueGold/15 pl-4 text-xs leading-6 text-moon/38">{locale === "en" ? "Context: " : "补充背景："}{context}</p>}
  ...
  return (
    <section ref={scope} className="relative overflow-hidden py-16 text-center md:py-20" aria-live="polite">
      ...
      <div className="relative mx-auto -mt-2 h-20 max-w-lg">
        {[
          locale === "en" ? "Sensing your question" : "正在感应你的问题",
          locale === "en" ? `Listening to how ${cardCount} cards answer each other` : `正在聆听 ${cardCount} 张牌之间的呼应`,
          locale === "en" ? "Returning the cards to your real situation" : "正在将牌意放回你的现实处境",
        ].map((label) => (
        ...
        <article className="min-w-0 pb-24">
          ...
          {context && (
            <details className="-mt-6 mb-8 border-b border-antiqueGold/10 pb-6 text-moon/40">
              <summary className="cursor-pointer text-[10px] uppercase tracking-[.22em] text-antiqueGold/45">{locale === "en" ? "View this reading's context" : "查看本次补充背景"}</summary>
          ...
          <section className="pb-10">
            ...
            <p className="mt-4 max-w-2xl font-zhSerif text-[16px] leading-8 tracking-[.025em] text-moon/64 md:text-[17px]">{locale === "en" ? "Start with each card's immediate voice. The full reading will form below." : "先看每张牌最直接的声音，完整解读会在下方慢慢成形。"}</p>
            <div className="mt-8">
                  ...
                  <div key={selected.positionId} className="grid grid-cols-[2rem_minmax(0,1fr)] gap-3 border-t border-antiqueGold/10 py-5 first:border-t-0">
                    ...
                    <div>
                      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        ...
                        <span className="text-[10px] tracking-[.16em] text-lavender/50">{selected.orientation === "reversed" ? locale === "en" ? "reversed" : "逆位" : locale === "en" ? "upright" : "正位"}</span>
                      </div>
                      <p className="mt-1 text-[10px] tracking-[.16em] text-antiqueGold/55">{locale === "en" ? position.title : position.titleZh} · {keywords.slice(0, 3).join(" · ")}</p>
          ...
          {!loading && !structured && !error && (
            <p className="py-12 font-zhSerif text-lg text-moon/45">{locale === "en" ? "The meaning is still settling. Please try again in a moment." : "牌意仍在沉淀，请稍候再试。"}</p>
          )}
